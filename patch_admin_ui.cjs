...
// Insert it right after the grid of 4 overview cards
const marker = '        {/* Total Sedang Cuti */}';
const parts = content.split(marker);
if (parts.length === 2) {
  // Find the closing div of the "Total Sedang Cuti" card
  const subParts = parts[1].split('</div>\n      </div>');
  if (subParts.length >= 2) {
      const replaced = parts[0] + marker + subParts[0] + '</div>\n      </div>\n' + uiInsertionStr + subParts.slice(1).join('</div>\n      </div>');
      fs.writeFileSync('src/components/DashboardView.tsx', replaced);
      console.log('UI Patched successfully');
  } else {
      console.log('Could not find closing div of the grid');
  }
} else {
