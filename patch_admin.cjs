...
// 1. Add Data calculations
const dataInsertionStr = `
...
// 2. Add UI for the Admin Visualizations
// Look for the end of Complete Metrics Overview Cards
// We can inject right after the grid
...
const gridEndRegex = /<div className="p-4 rounded-3xl bg-white\/70 dark:bg-slate-900\/60 backdrop-blur-2xl border border-white\/60 dark:border-white\/10 shadow-lg">[\s\S]*?Menunggu Persetujuan[\s\S]*?<\/div>[\s\S]*?<\/div>\s+<div className="mt-4 sm:mt-6 grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">/;
