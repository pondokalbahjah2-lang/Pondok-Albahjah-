        ...
        <div className="flex flex-col gap-4 lg:gap-6">
          ...
          <div className="p-6 rounded-3xl bg-white/70 dark:bg-slate-900/60 backdrop-blur-2xl border border-white/60 dark:border-white/10 shadow-xl flex-1">
            <h3 className="text-md font-bold text-slate-800 dark:text-white mb-4 flex items-center gap-2">
              ...
              Divisi Paling Sering Absen (Keseluruhan)
            </h3>
            <div className="h-40">
...
// Insert it right before the "Statistik 6 Bulan Terakhir" section
const marker = '        {/* ---------------- Advanced Charts (6 Months & Departement) ---------------- */}';
const parts = content.split(marker);
if (parts.length === 2) {
  const replaced = parts[0] + uiInsertionStr + '\n' + marker + parts[1];
  fs.writeFileSync('src/components/DashboardView.tsx', replaced);
  console.log('UI Patched successfully with marker Advanced Charts');
} else {
  console.log('Could not find marker Advanced Charts');
}
