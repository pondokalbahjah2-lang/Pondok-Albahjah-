...
export function useTeachingData(currentUser: UserAccount | null) {
  ...
  return {
    ...
    teachingSettings: {
      ...settings,
      cutoffStartDay: settings.cutOffDay ?? 26,
      cutoffEndDay: settings.cutOffDay ? settings.cutOffDay - 1 : 25
    },
    getWIBDate: () => {
    ...
    generateTeachingReport: (opts: { startDate: string; endDate: string; unit?: string; pejuangId?: string }) => {
      return {
